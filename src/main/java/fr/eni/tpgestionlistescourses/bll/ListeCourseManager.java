...
import fr.eni.tpgestionlistescourses.dal.ListeCourseDAO;
...
public class ListeCourseManager
{
	...
	{
		if ((nomListe == null) || (nomListe.trim().length() == 0) || (nomListe.trim().length() > 50))
		{
	...
	{
		if ((nomArticle == null) || (nomArticle.trim().length() == 0) || (nomArticle.trim().length() > 50))
		{
